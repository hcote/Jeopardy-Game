$(document).ready(function() {
    ...
    // For getting the dollar value without the $ symbol
    // Regular expression can also be created with: /[0-9]/+
    ...
    // Note: Noticed that if I did NOT click the button, and instead clicked on
    // one of the boxes, it took the box off the page but offered no question
    // or anything.

    // So how do we dry this up? Review the steps:
    // 1. Get 5 random categories
    // 2. Use categories to get 5 random clues for each
    // 3. Store clues in object for easy access and minimal requests
    // 4. Add the clues to each box on the game board
    ...
    // 3. you could make 1 request every time a box is clicked.
    $('.box').on('click', function(e) {
        $.ajax({
            ...
            data: {
                ...
                value: valuePattern.exec($(this).text())[0],  // Use our regex to get value from html
                offset: 5       // Sends 1 clue for this category and value
            },
            success: showClue,
            error: getErrors
        })
